...
export async function transcribeAudio(audioAsBase64: string, mimeType: string) {
    const response = await gemini.models.generateContent({
        ...
        contents: [
            {
                text: 'Transcreva o audio para português do Brasil. Seja preciso e natural na transcrição. Matenha a pontuação adequada e divida o texto em parágrafos quando for apropriado.'
            },
            {
                inlineData: {
                    mimeType,
...
export async function generateEmbeddings(text: string) {
    const response = await gemini.models.embedContent({
        ...
        contents: [{text}],
        config: {
            taskType: 'RETRIEVAL_DOCUMENT', //"taskType" diz a finalidade do embedding e o "RETRIEVAL_DOCUMENT" é para fazer buscas semânticas e utilizar o conteúdo em outros prompts
        }
    })
